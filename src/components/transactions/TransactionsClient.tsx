...
export function TransactionsClient({ initialData }: TransactionsClientProps) {
  ...
  const [mobileTransactions, setMobileTransactions] = useState<Transaction[]>(initialData.data);
  ...
  // Use initialData for first render, then switch to query data
  const displayTransactions = transactions.length > 0 ? transactions : initialData.data;
  const displayCount = totalCount > 0 ? totalCount : initialData.count;
